...
from nlp_class2.util import init_weight, get_ptb_data, display_tree
...
class RNTN:
        ...
        with tf.Session() as session:
            ...
            for i in range(epochs):
                ...
                for train_op, cost, prediction, labels in zip(train_ops, costs, predictions, all_labels):
                    _, c, p = session.run([train_op, costs, prediction])
                    epoch_cost += c
    ...
    def get_cost(self, logits, labels, , reg):
        cost = tf.reduce_mean(tf.nn.sparse_softmax_cross_entorpy_with_logits(logits, labels))
        rcost = sum(tf.nn.l2_loss(p) for p in self.params)
        cost += reg*rcost
        return cost

        # list_of_logits is an output!
        # list_of_logits object passed by reference
